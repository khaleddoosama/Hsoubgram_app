<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div class="flex items-center gap-4">
            <img src="{{ Str::startsWith($user->image,'https') ? $user->image : asset('storage/'.$user->image) }}" class="w-24 h-24 rounded-full object-cover">
        </div>
        ...
        <div>
            ...
            <textarea name="bio" id="bio"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('bio', $user->bio) }}</textarea>
            <x-input-error class="mt-2" :messages="$errors->get('username')" />
        </div>
        ...
        <label class="inline-flex items-center cursor-pointer">
            <input type="checkbox" name="private_account" id="private_account" class="sr-only peer" {{ $user->private_account ? 'checked' : '' }}>
            <div
